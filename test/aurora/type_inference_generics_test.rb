...
class TypeInferenceGenericsTest < Minitest::Test
  def test_simple_generic_function_call
    ...
    core_ir, _ = Aurora.transform_to_core_with_registry(Aurora.parse(source))

    # Find main function
    main_func = core_ir.items.find { |item| item.is_a?(Aurora::CoreIR::Func) && item.name == 'main' }
    refute_nil main_func

    # Body should be a call to identity
    assert_instance_of Aurora::CoreIR::CallExpr, main_func.body
  ...
  def test_generic_array_function
    ...
    assert main_func.body.is_a?(Aurora::CoreIR::BlockExpr)
  end
  ...
  def test_type_inference_consistency_check
    ...
    assert_raises(Aurora::CompileError) do
      Aurora.transform_to_core_with_registry(Aurora.parse(source))
    end
  end
