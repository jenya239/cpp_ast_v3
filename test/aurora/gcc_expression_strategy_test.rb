...
class GccExpressionStrategyTest < Minitest::Test
  ...
  def test_gcc_vs_iife_block_complexity
    ...
    simple_block = Aurora::CoreIR::BlockExpr.new(
      ...
      result: Aurora::CoreIR::VarExpr.new(name: "x", type: int_type),
      type: int_type
    )

    analyzer = Aurora::Backend::BlockComplexityAnalyzer.new(simple_block)
    assert analyzer.simple?
    refute analyzer.complex?

    # With GCC policy, simple blocks use gcc_expr
    gcc_policy = Aurora::Backend::RuntimePolicy.gcc_optimized
  ...
  def test_scope_tmp_falls_back_to_gcc_when_enabled
    ...
    policy_with_gcc = Aurora::Backend::RuntimePolicy.new
