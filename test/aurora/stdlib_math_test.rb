...
class StdlibMathTest < Minitest::Test
  def test_math_module_parses
    source = File.read(File.expand_path('../../lib/aurora/stdlib/math.aur', __dir__))
  ...
  def test_math_extern_functions_are_marked_external
    ...
    sqrt_f = ast.declarations.find { |d| d.is_a?(Aurora::AST::FuncDecl) && d.name == 'sqrt_f' }
  ...
  def test_math_helper_functions_not_external
    ...
    hypotenuse = ast.declarations.find { |d| d.is_a?(Aurora::AST::FuncDecl) && d.name == 'hypotenuse' }
  ...
  def test_export_extern_combination
    ...
    assert func.is_a?(Aurora::AST::FuncDecl)
