...
class OpaqueTypeTest < Minitest::Test
  def test_opaque_type_parsing
    ...
    assert_instance_of Aurora::AST::TypeDecl, window_decl
    assert_equal "Window", window_decl.name
    assert_instance_of Aurora::AST::OpaqueType, window_decl.type
    assert window_decl.exported

    ctx_decl = ast.declarations[1]
    assert_instance_of Aurora::AST::TypeDecl, ctx_decl
    assert_equal "DrawContext", ctx_decl.name
    assert_instance_of Aurora::AST::OpaqueType, ctx_decl.type
  end
  ...
  def test_opaque_type_core_ir_transformation
    ...
    # CoreIR type should be OpaqueType
    assert_instance_of Aurora::CoreIR::OpaqueType, type_info.core_ir_type
  end
  ...
  def test_opaque_type_builder
    opaque = Aurora::CoreIR::Builder.opaque_type("MyType")

    assert_instance_of Aurora::CoreIR::OpaqueType, opaque
