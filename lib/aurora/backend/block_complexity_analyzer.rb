  ...
  module Backend
    ...
    class BlockComplexityAnalyzer
      ...
      def statement_has_control_flow?(stmt)
        case stmt
        when CoreIR::IfStmt, CoreIR::MatchStmt, CoreIR::ForStmt, CoreIR::WhileStmt
          true
        when CoreIR::VariableDeclStmt
          stmt.value && expression_has_control_flow?(stmt.value)
        when CoreIR::ExprStmt
      ...
      def expression_has_control_flow?(expr)
        case expr
        when CoreIR::IfExpr, CoreIR::MatchExpr
          true
      ...
      def has_nested_blocks?
        @block_expr.statements.any? do |stmt|
          stmt.is_a?(CoreIR::VariableDeclStmt) && stmt.value.is_a?(CoreIR::BlockExpr)
        end
      end
    end
    ...
    class IfComplexityAnalyzer
      ...
      def has_nested_control_flow?
        [*@if_expr.then_branch, @if_expr.else_branch].compact.any? do |branch|
          branch.is_a?(CoreIR::IfExpr) ||
            branch.is_a?(CoreIR::MatchExpr) ||
            branch.is_a?(CoreIR::BlockExpr)
        end
      end
    end
